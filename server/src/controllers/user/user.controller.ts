...
class UserController {
    ...
    public verifyEmail = catchAsync(async (req: Request, res: Response) => {
        const verifictionToken = req.params.token
        ...
        jwt.verify(
            ...
            async (err: VerifyErrors | null, decoded: unknown) => {
                ...
                try {
                    ...
                    userService.findUserByVerificationToken(email, verifictionToken)
                    .then((user) => {
                        if(!user || user.isVerified) {
                            return res.sendStatus(400)
                        }

                        userService.updateIsVerified(user, true).then(() => {return res.sendStatus(200)}).catch(() => {
